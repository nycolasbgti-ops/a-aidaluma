...
function buildWAMessage(order) {
  ...
  const lines = (order.items || []).map(i => {
    const qty  = i.qty || 1
    const name = i.crustLabel ? `${i.name} (+ ${i.crustLabel})` : i.name
    return `• ${qty}× ${name} — ${fmt(i.price * qty)}`
  }).join('\n')
...
export default function ConfirmationView({ order, onNewOrder }) {
  ...
  if (order.payment_method === 'pix') {
    return (
      <div className="min-h-screen bg-[#0A0A0A] text-white flex flex-col items-center justify-center px-5 py-10">
        <div className="max-w-sm w-full text-center">

          {/* Ícone animado dourado */}
          <div className={`w-28 h-28 rounded-full flex items-center justify-center mx-auto mb-7
                           transition-all duration-500 ${tick ? 'bg-[#D4AF37]/20 scale-100' : 'scale-50 opacity-0'}`}>
            <span className={`text-5xl transition-all duration-500 delay-200 ${tick ? 'opacity-100' : 'opacity-0'}`}>
              ⏳
            </span>
          </div>

          <h1 className="text-3xl font-extrabold mb-2 text-[#D4AF37]">Aguardando Pagamento!</h1>
          ...
          {/* Resumo do pedido */}
          <div className="bg-[#1A1A1A] rounded-2xl p-4 mb-5 text-left">
            ...
            <div className="flex justify-between pt-3 mt-1 border-t border-white/5 font-bold">
              <span>Total</span>
              <span className="text-xl text-[#D4AF37]">{fmt(order.total)}</span>
            </div>
            <div className="mt-3 pt-3 border-t border-white/5 flex flex-wrap gap-2">
              <span className="text-xs bg-[#242424] px-2.5 py-1 rounded-full text-gray-400">
                {order.delivery_type === 'delivery' ? '🛵 Entrega' : '🏪 Retirada'}
              </span>
              <span className="text-xs bg-[#242424] px-2.5 py-1 rounded-full text-gray-400">💠 Pix</span>
            </div>
          </div>

          {/* Caixa da chave Pix */}
          <div className="bg-[#1A1A1A] border border-[#D4AF37]/30 rounded-2xl p-4 mb-5 text-left">
            ...
            <div className="flex items-center gap-2 mb-2.5">
              ...
              <button
                ...
                className={`flex-shrink-0 px-4 py-2.5 rounded-xl font-bold text-sm transition-all active:scale-95 ${
                  copied ? 'bg-green-500 text-white' : 'bg-[#D4AF37] text-[#0A0A0A]'
                }`}
              >
            ...
            <p className="text-xs text-gray-500 leading-relaxed">
              ...
              O valor a pagar é <span className="text-[#D4AF37] font-semibold">{fmt(order.total)}</span>.
            </p>
          </div>
          ...
          <button
            onClick={onNewOrder}
            className="w-full py-3.5 bg-[#1A1A1A] rounded-2xl font-semibold text-gray-400
